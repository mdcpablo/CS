...
def power_iterations(mesh, bc, problem_type, discretization, mode='normal', L_max=9, S_tol=1e-6, F_tol=1e-6, max_its=1e3, tol=1e-6, talk=True, k_exact=None, recomp_F=1, recomp_S=[1,1,1,1,1,1,1,1,1], DSA_opt=True):
        ...
        for g in range(G):
            ...
            for m in range(N):
                if m == 0:
                    ...
                    psi_L[g], psi_hat_down[g,0,:] = sweep_diamond_starting_direction(mesh, g, Q_start, psi_R[g])
                else:
                    psi_hat_down[g,m,:] = np.copy(psi_hat_up[g,m-1,:])
                psi[g,m,:], psi_left[g,m,:], psi_right[g,m,:], psi_hat_up[g,m,:] = sweep_diamond(mesh, g, m, Q[g,m,:], psi_hat_down[g,m,:], psi_L[g], psi_R[g])
                count += I #15*I
    ...
    def sweep_diamond(mesh, g, m, Q, psi_hat_down, psi_L, psi_R):
