...
mg_k, mg_k_error = run_dofs('HMF001_',dofs,'mg.xml',I=2,N=2,L_max=1,note='MG')
oldfeds_k, oldfeds_k_error = run_dofs('HMF001_',dofs,'oldfeds_5EperCG.xml',I=2,N=2,L_max=1,note='old FEDS (5 elements per coarse group)')
newfeds_k, newfeds_k_error = run_dofs('HMF001_',dofs,'newfeds.xml',I=2,N=2,L_max=1,note='new FEDS')
# -----------------------------------------------------------------------------
...
plt.show()
# -----------------------------------------------------------------------------
plt.loglog(dofs, mg_k_error, 'bo', label='MG')
plt.loglog(dofs, oldfeds_k_error, 'ro', label='old FEDS')
plt.loglog(dofs, newfeds_k_error, 'go', label='new FEDS')
plt.xlabel(r'Number of Energy Groups')
